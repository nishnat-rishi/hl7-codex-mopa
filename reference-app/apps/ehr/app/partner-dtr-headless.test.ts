...
describe("headless partner CRD → DTR → next CRD handoff", () => {
  it("retains order/coverage context when a text answer returns from DTR", async () => {
    ...
    const coverage = { resourceType: "Coverage", id: "mopa-connectathon-coverage",
      status: "active", beneficiary: { reference: "Patient/sandra-chen" } };
    const canonical = "https://mock.oncohealth.example/fhir/Questionnaire/breast-cancer-regimen|2026.1-mock";
    const packagePayload = { resourceType: "Parameters", parameter: [{
      name: "packagebundle", resource: { resourceType: "Bundle", type: "collection", entry: [
        { resource: { resourceType: "Questionnaire", url: canonical.split("|")[0],
          version: "2026.1-mock", item: [{ linkId: "priorTherapy",
            text: "Prior systemic therapy and response", type: "text", required: true }] } },
        { resource: { resourceType: "QuestionnaireResponse", questionnaire: canonical,
          subject: { reference: "Patient/sandra-chen" }, extension: [
    ...
    vi.stubGlobal("fetch", vi.fn(async (input: string, init?: RequestInit) => {
      requests.push({ url: input, body: init?.body && !input.endsWith("/oauth/token")
        ? JSON.parse(String(init.body)) : undefined });
      const payload = input.endsWith("/oauth/token")
        ? { access_token: "synthetic-test-token", expires_in: 3600 }
        : input.endsWith("/Questionnaire/$questionnaire-package") ? packagePayload
        : input.endsWith("/Coverage/mopa-connectathon-coverage") ? coverage
        : input.endsWith("/Patient/sandra-chen") ? { resourceType: "Patient", id: "sandra-chen" }
        : { resourceType: "Bundle", type: "searchset", entry:
            input.includes("QuestionnaireResponse?") && savedResponse
    ...
    const payload = await buildPartnerDtrPackage({
      contextId: "mopa-ctx-123", patientId: "sandra-chen",
    ...
    const outbound = requests.find((request) => request.url.endsWith("/Questionnaire/$questionnaire-package"));
    expect(outbound?.body).toMatchObject({ resourceType: "Parameters", parameter: [
    ...
    expect(rendered).toMatchObject({ items: [{ linkId: "priorTherapy", type: "text" }],
      contextReference: "RequestGroup/rg-PHD",
      coverageReference: "Coverage/mopa-connectathon-coverage" });
    if (!rendered) throw new Error("Partner questionnaire did not render");
    savedResponse = buildQuestionnaireResponse("sandra-chen",
      { priorTherapy: "No prior systemic therapy." }, rendered.items,
      "2026-09-20", rendered.canonical, rendered.contextReference, rendered.coverageReference);
    const prefetch = await buildPartnerPrefetch("sandra-chen");
    expect(prefetch.questionnaireResponseBundle).toMatchObject({ entry: [{ resource: {
    ...
    expect(requests.some((request) => request.url.includes(
      "QuestionnaireResponse?subject=Patient/sandra-chen"))).toBe(true);
  });
});
